...
function Avatar({ src, label, size = 42 }: { src?: string | null; label: string; size?: number }) {
  ...
  // Browser-native media is deliberate for signed Supabase URLs and WebKit stability.
...
function Caption({ value }: { value: string }) {
  ...
  const stopLinkBubble = (event: MouseEvent<HTMLAnchorElement>) => event.stopPropagation();
  return (
    <p className="caption">
      {parts.map((part, index) => {
        if (/^https?:\/\//i.test(part)) {
          return <a className="linkish" href={part} target="_blank" rel="noreferrer" onClick={stopLinkBubble} key={`${part}-${index}`}>{part}</a>;
        }
...
function FeedPost({
  ...
  return (
    <article className="post parity-feed-post">
      <button className="avatar-button" type="button" aria-label={`เปิดโปรไฟล์ ${authorLabel(row)}`} onClick={() => onProfile(row.author_id)}>
      ...
      <div className="post-main">
        ...
        <div className="post-header">
          <button className="author-link" type="button" onClick={() => onProfile(row.author_id)}>
        ...
        <div className="post-open-button" role="button" tabIndex={0} onClick={() => onOpen(row)} onKeyDown={(event) => activateOnKeyboard(event, () => onOpen(row))}>
  ...
  return (
    <article className="post parity-feed-post club-home-card">
      ...
      <div className="post-main">
        ...
        <div className="post-open-button" role="button" tabIndex={0} onClick={() => onOpen(post)} onKeyDown={(event) => activateOnKeyboard(event, () => onOpen(post))}>
          {post.content ? <Caption value={post.content} /> : null}
          {post.image_urls[0] ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img className="post-media" src={post.image_urls[0]} alt="" loading="lazy" decoding="async" />
          ) : null}
...
function SideDrawer({ identity, userId, onClose }: { identity: HomeIdentity | null; userId: string; onClose: () => void }) {
  ...
  const rows = [
    ...
    { label: "สร้าง Club", icon: Plus, href: "/search?club=create" },
    { label: "Club ของฉัน", icon: UsersRound, href: "/search?club=mine" },
    { label: "บันทึกไว้", icon: Bookmark, href: `/profile/${userId}?tab=saved` },
  ];
  return (
...
export function ParityHome({ session }: { session: Session }) {
  ...
  const loadIdentity = useCallback(async () => {
    ...
    try {
      ...
      // Secondary chrome must never block the feed.
    }
  }, [supabase, userId]);
  ...
  const loadFeed = useCallback(async (nextMode: FeedMode) => {
    ...
    try {
      if (nextMode === "clubs") {
        const nextClubPosts = await fetchClubHomePosts(supabase, userId);
        setRows([]);
        setViewer(emptyViewerState());
        setClubPosts(nextClubPosts);
      } else {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      void loadIdentity();
      void loadFeed("for-you");
    }, 0);
    return () => window.clearTimeout(timer);
  }, [loadFeed, loadIdentity]);
  ...
  const patchCount = (id: string, field: "like_count" | "redrop_count", delta: number) => {
    setRows((current) => current.map((row) => row.id === id ? { ...row, [field]: Math.max(0, (row[field] ?? 0) + delta) } : row));
  };
  ...
  const onLike = async (row: HomeFeedRow) => {
    ...
    patchCount(row.id, "like_count", wasLiked ? -1 : 1);
    try {
  ...
  const onRedrop = async (row: HomeFeedRow) => {
    ...
    patchCount(row.id, "redrop_count", wasRedropped ? -1 : 1);
    try {
  ...
  const onFollow = async (row: HomeFeedRow) => {
    if (!supabase || row.author_id === userId) return;
    ...
    if (isPrivate && wasRequested && !window.confirm(`ยกเลิกคำขอติดตาม ${authorLabel(row)}?`)) {
      endPending(key);
      return;
    }
    try {
      const state = await toggleAuthorFollow(supabase, userId, row.author_id, {
        currentlyFollowing: wasFollowing,
        pendingRequest: wasRequested,
        isPrivate,
      });
      setViewer((current) => {
        let next = withViewerSet(current, "followedAuthorIds", row.author_id, state === "following");
        next = withViewerSet(next, "pendingFollowAuthorIds", row.author_id, state === "requested");
        return next;
      });
    } catch {
  ...
  const onClubLike = async (post: ClubHomePost) => {
    if (!supabase) return;
    const key = `club-like:${post.id}`;
    if (!beginPending(key)) return;
    setClubPosts((current) => current.map((item) => item.id === post.id ? {
      ...item,
      liked_by_me: !post.liked_by_me,
      like_count: Math.max(0, post.like_count + (post.liked_by_me ? -1 : 1)),
    } : item));
    try {
      await toggleClubPostLike(supabase, userId, post.id, post.liked_by_me);
    } catch {
      setClubPosts((current) => current.map((item) => item.id === post.id ? post : item));
      showToast("อัปเดตการถูกใจไม่สำเร็จ");
    } finally {
      endPending(key);
    }
  ...
  const onClubSave = async (post: ClubHomePost) => {
    if (!supabase) return;
    const key = `club-save:${post.id}`;
    if (!beginPending(key)) return;
    setClubPosts((current) => current.map((item) => item.id === post.id ? { ...item, saved_by_me: !post.saved_by_me } : item));
    try {
      await toggleClubPostSave(supabase, userId, post.id, post.saved_by_me);
    } catch {
      setClubPosts((current) => current.map((item) => item.id === post.id ? post : item));
      showToast("อัปเดตที่บันทึกไม่สำเร็จ");
    } finally {
  ...
  const onTouchEnd = (event: TouchEvent) => {
    ...
    const index = feedModes.findIndex((item) => item.id === mode);
    const next = delta < 0 ? Math.min(feedModes.length - 1, index + 1) : Math.max(0, index - 1);
    if (next !== index) selectMode(feedModes[next].id);
  };
      ...
      <main className="wynos-main parity-home-main" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
        <header className="parity-home-shell">
          <div className="parity-home-header">
            ...
            <div className="home-wordmark" aria-label="WYNOS">
              {/* Exact Flutter asset, copied byte-for-byte from app/assets/images. */}
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img className="home-logo-mark" src="/wynos_logo_mark.png" alt="" />
              <strong>WYNOS</strong>
            </div>
        ...
        <section className="home-feed-surface" aria-live="polite">
          {loading ? <div className="home-loading"><div className="route-system-spinner" aria-label="กำลังโหลด" /></div> : mode === "clubs" ? (
            clubPosts.length ? clubPosts.map((post) => (
              <ClubPostCard
                post={post}
                pending={pending}
                onLike={(value) => void onClubLike(value)}
                onSave={(value) => void onClubSave(value)}
                onOpen={(value) => router.push(`/club-post/${value.id}`)}
                key={post.id}
              />
            )) : <div className="home-empty"><p>เข้าร่วม Club เพื่อดูโพสต์ที่นี่</p><button className="route-secondary" type="button" onClick={() => router.push("/search")}>สำรวจ Club</button></div>
            ...
            <FeedPost
              ...
              onOpen={(value) => router.push(`/drop/${value.id}`)}
              onProfile={(profileId) => router.push(`/profile/${profileId}`)}
              key={`${row.id}:${row.redrop_id ?? "plain"}`}
            />
          )) : <div className="home-empty">ยังไม่มีโพสต์ที่แสดงได้ในฟีดนี้</div>}
        </section>
      </main>
        ...
        <div className="sheet-backdrop" role="presentation" onClick={() => !publishing && setComposerOpen(false)}>
          <section className="sheet composer-sheet parity-composer" role="dialog" aria-modal="true" aria-label="สร้าง Drop" onClick={(event) => event.stopPropagation()}>
            <header className="sheet-header">
              ...
              <button className="share-button" type="button" disabled={publishing || (!composerCaption.trim() && !composerFiles.length)} onClick={() => void submitDrop()}>{publishing ? "กำลังแชร์…" : "แชร์"}</button>
            </header>
            <div className="composer-body">
              <textarea autoFocus value={composerCaption} maxLength={500} placeholder="มีอะไรอยาก Drop ไหม?" onChange={(event) => { setComposerCaption(event.target.value); setPublicationOperationId(null); }} />
              <div className="composer-tools">
                <label className="image-picker"><ImagePlus size={22} /><span>เพิ่มรูป</span><input type="file" accept="image/*" multiple disabled={publishing} onChange={(event) => { setComposerFiles(Array.from(event.target.files ?? []).slice(0, 9)); setPublicationOperationId(null); }} /></label>
                <span>{composerCaption.length}/500</span>
              </div>
              {composerFiles.length ? <div className="selected-files"><strong>รูปที่เลือก {composerFiles.length}/9</strong>{composerFiles.map((file) => <span key={`${file.name}:${file.size}`}>{file.name}</span>)}</div> : null}
              {publicationOperationId ? <p className="publication-retry-note">ระบบจะลองเผยแพร่รายการเดิมอีกครั้งอย่างปลอดภัย โดยไม่สร้าง Drop ซ้ำ</p> : null}
            </div>
          </section>
        </div>
      ) : null}
