...
import { Bookmark, Eye, Flag, Heart, MessageCircle, MoreHorizontal, Quote, Repeat2, Share2 } from "lucide-react";
...
export function GoldenDropCard({ row, homeParity = false }: { row: HomeFeedRow; homeParity?: boolean }) {
  ...
  return <article className="golden-drop-card">
    {row.redrop_id ? <div className="golden-drop-redrop"><Repeat2 size={homeParity ? 16 : 13} />รีโพสต์โดย {homeParity ? "" : "@"}{row.redropper_username || "wynos"} · {relativeTimeTh(row.created_at)}</div> : null}
    ...
    <div className="golden-drop-body">
      ...
      {homeParity ? (
        ...
        <div className="golden-drop-actions"><button className={liked ? "liked" : ""} type="button" aria-label={liked ? "เลิกถูกใจ" : "ถูกใจ"} onClick={() => void like()}><AnimatedHeart size={22} strokeWidth={2} liked={liked} />{likeCount > 0 ? <span>{likeCount}</span> : null}</button><Link href={`/drop/${row.id}#comments`} aria-label="ความคิดเห็น"><MessageCircle size={22} strokeWidth={2} />{(row.comment_count ?? 0) > 0 ? <span>{row.comment_count}</span> : null}</Link>{canRedrop ? <button className={redropped ? "active" : ""} type="button" aria-label="รีโพสต์" onClick={() => setSheet("redrop")}><Repeat2 size={24} strokeWidth={2} />{redropCount > 0 ? <span>{redropCount}</span> : null}</button> : null}<button className="golden-drop-share" type="button" aria-label="แชร์" onClick={() => void share()}><WynosShareIcon size={24} /></button><button className={`golden-drop-save-inline ${saved ? "active" : ""}`} type="button" aria-label={saved ? "ยกเลิกบันทึก" : "บันทึก"} aria-pressed={saved} onClick={() => void save()}><Bookmark size={22} strokeWidth={2} fill={saved ? "currentColor" : "none"} /></button>{viewCount != null ? <span className="golden-drop-view"><Eye size={22} />{viewCount > 0 ? <span>{viewCount}</span> : null}</span> : null}</div>
      )}
    </div>

    {sheet === "more" ? <SheetFrame label="ตัวเลือกโพสต์" onClose={() => setSheet(null)}><button className="golden-drop-sheet-row" type="button" onClick={() => { setSheet(null); void share(); }}><Share2 size={20} />แชร์</button><button className="golden-drop-sheet-row" type="button" onClick={() => void save()}><Bookmark size={20} fill={saved ? "currentColor" : "none"} />{saved ? "เอาออกจากบันทึก" : "บันทึก"}</button>{!own ? <button className="golden-drop-sheet-row" type="button" onClick={() => setSheet("report")}><Flag size={20} />รายงานโพสต์</button> : null}</SheetFrame> : null}
    {sheet === "redrop" ? <SheetFrame label="รีโพสต์" onClose={() => setSheet(null)}><button className="golden-drop-sheet-row" type="button" onClick={() => void redrop()}><Repeat2 size={20} />{redropped ? "ยกเลิก ReDrop" : "ReDrop"}</button><button className="golden-drop-sheet-row" type="button" onClick={() => setSheet("quote")}><Quote size={20} />Quote ReDrop</button></SheetFrame> : null}
