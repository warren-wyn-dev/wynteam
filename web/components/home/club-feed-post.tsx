 ...
 * (shares `post-card.module.css` / `WynosPostCard`), minus ReDrop/Share/
 * Follow, which the club feed doesn't have. Per the same DOM correction as
 * HomePostCard: media (when present) renders outside `.post-body`, and its
 * actions row moves to `.post-footer` below the media instead of staying
 * inside the body. No swipe/dots indicator here (unlike HomePostCard's
 * carousel) — club posts don't track a scroll position, so a static dot
 * row would misleadingly always show the first dot active. */
export function ClubFeedPost({ post, onLike }: { post: ClubHomePost; onLike: () => void }) {
  ...
  const media = post.image_urls.length ? (
    <div className={`${pc.mediaTrack} ${pc.mediaTrackFeed} ${post.image_urls.length === 1 ? pc.mediaTrackSingle : ""}`}>
  ...
  return (
    <WynosPostCard
      ...
      caption={caption}
      media={media}
      mediaCount={post.image_urls.length}
      showDots={false}
      actions={actions}
    />
  );
