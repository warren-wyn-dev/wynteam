...
function ConversationInner({ client, userId, conversationId }: { client: SupabaseClient; userId: string; conversationId: string }) {
  ...
  const refresh = useCallback(async () => {
    ...
    // A transient read-receipt failure must not discard a successfully loaded thread.
  ...
  const submit = async () => {
    ...
    try {
      ...
      // Delivery is confirmed: a failed read receipt is best-effort, not a send error.
