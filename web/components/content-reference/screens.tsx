...
export function HomeReferenceScreen() {
  ...
  const [liked, setLiked] = useState<Record<string, boolean>>(
    Object.fromEntries(referencePosts.map((post) => [post.id, post.liked])),
  );
    ...
    <ReferencePhone>
      <div className="home-head">
        <div className="home-head-row">
          <button className="ic-btn" onClick={() => router.push("/settings")} aria-label="การตั้งค่า" type="button">
            <WynosIcon name="menu" size={21} />
          </button>
          <div className="brand-lockup">
            <svg height="22" viewBox="0 0 26 26" width="22" aria-label="Wynos"><path d="M2 4 L8 22 L13 9 L18 22 L24 4" fill="none" stroke="#0A0A0A" strokeLinecap="round" strokeLinejoin="round" strokeWidth="3.4" /></svg>
            <span>Wynos</span>
          </div>
          <div className="home-head-actions">
            <button className="ic-btn" onClick={() => router.push("/search")} aria-label="ค้นหา" type="button"><WynosIcon name="search" size={21} /></button>
            <button className="ic-btn" onClick={() => router.push("/notifications")} aria-label="การแจ้งเตือน" type="button"><WynosIcon name="notifications" size={21} /></button>
          </div>
        </div>
      ...
      <BottomNav active="home" className="bottomnav" homeHref="/home" clubsHref="/clubs" postHref="/compose-post" chatHref="/chat" profileHref="/profile" />
    </ReferencePhone>
  );
}
...
export function ComposePostReferenceScreen() {
  ...
  return (
    <ReferencePhone>
      <div className="topbar">
        <button className="ic-btn" onClick={() => router.push("/home")} aria-label="ปิด" type="button"><WynosIcon name="close" size={20} /></button>
        <span className="draft-label">ฉบับร่าง</span>
        <button className="compose-submit" onClick={() => router.push("/home")} type="button">โพสต์</button>
      </div>
      <div className="compose-body">
        <Avatar as="div" alt="รูปโปรไฟล์" className="avatar" size={36} />
        <div className="compose-editor">
          <textarea placeholder="มีอะไรจะพูดไหม" />
          {mode === "poll" ? <div className="compose-mode-panel" data-compose-mode="poll"><Input bare aria-label="ตัวเลือกโพล 1" placeholder="ตัวเลือก 1" /><Input bare aria-label="ตัวเลือกโพล 2" placeholder="ตัวเลือก 2" /></div> : null}
          {mode === "image" ? <div className="compose-mode-panel" data-compose-mode="image"><label className="image-picker"><span>{imageName || "เลือกรูปภาพ"}</span><input accept="image/*" onChange={onImageChange} type="file" /></label></div> : null}
        </div>
      </div>
      <div className="compose-toolbar"><div className="compose-tools">
        <button aria-label="แนบรูป" aria-pressed={mode === "image"} onClick={() => toggleMode("image")} type="button"><WynosIcon name="image" size={21} /></button>
        <button aria-label="สร้างโพล" aria-pressed={mode === "poll"} onClick={() => toggleMode("poll")} type="button"><WynosIcon name="poll" size={21} /></button>
        <button aria-label="ตำแหน่ง" onClick={() => setMode("text")} type="button"><WynosIcon name="location" size={21} /></button>
      </div></div>
    </ReferencePhone>
  );
}
    ...
    <ReferencePhone>
      ...
      <div className="single-scroll">
        <div className="single-post">
          <div className="single-author"><Avatar as="div" alt={`รูปโปรไฟล์ของ ${post.authorName}`} className="avatar" size={38} /><div className="single-author-copy"><div>{post.authorName}</div><span>{post.username}</span></div></div>
...
export function NotificationsReferenceScreen() {
  const router = useRouter();
  return (
    <ReferencePhone><BackTopbar title="การแจ้งเตือน" onBack={() => router.push("/home")} /><div className="notification-list">
      <div className="notification-row unread"><WynosIcon className="notification-kind liked" name="like" size={18} /><Avatar as="div" alt="ต้น สายเทค" className="avatar" size={30} /><div className="notification-copy"><p><b>ต้น สายเทค</b> กดใจโพสต์ของคุณ</p></div><span className="notification-time">2 ชม.</span></div>
      <div className="notification-row"><WynosIcon className="notification-kind" name="userPlus" size={18} /><Avatar as="div" alt="มายด์ กาแฟรัก" className="avatar" size={30} /><div className="notification-copy"><p><b>มายด์ กาแฟรัก</b> เริ่มติดตามคุณ</p></div><Button className="follow-back" variant="outline">ติดตามกลับ</Button></div>
    </div></ReferencePhone>
  );
}
