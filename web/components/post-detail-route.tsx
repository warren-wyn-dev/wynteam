  ...
  return (
    <AppChrome title="" userId={userId} headerMode="hidden" showBottomNav={false}>
      ...
      <article className="detail-post flutter-detail-post">
        ...
        <div className={`detail-actions flutter-detail-actions ${publicAudience ? "public" : "private"}`}><button className={liked ? "active like" : ""} type="button" aria-label={liked ? "เลิกถูกใจ" : "ถูกใจ"} aria-pressed={liked} onClick={() => void interact("like")}><AnimatedHeart size={24} strokeWidth={2} liked={liked} />{row.like_count ?? 0}</button><button type="button" aria-label="ความคิดเห็น" onClick={() => composerRef.current?.focus()}><CommentIcon size={24} strokeWidth={2} />{row.comment_count ?? 0}</button>{publicAudience ? <button className={redropped ? "active" : ""} type="button" aria-label={redropped ? "ยกเลิกรีโพสต์" : "รีโพสต์"} aria-pressed={redropped} onClick={() => void interact("redrop")}><RepostIcon size={24} strokeWidth={2} />{row.redrop_count ?? 0}</button> : null}<button type="button" aria-label="แชร์โพสต์" onClick={() => void share()}><WynosShareIcon size={24} /></button><button className={saved ? "active" : ""} type="button" aria-label={saved ? "นำออกจากที่บันทึก" : "บันทึกโพสต์"} aria-pressed={saved} onClick={() => void interact("save")}><SaveIcon size={24} strokeWidth={2} saved={saved} /></button></div>
