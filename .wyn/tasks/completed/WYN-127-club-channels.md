...
Status: **LIVE ใน production (2026-09-07)** — migration + deploy run #96 เสร็จโดย session ที่รับช่วงต่อจาก go-live package, แต่ Founder ทดลองใช้จริงด้วย developer account ใน Club "WYNOS Feedback" แล้วสั่งปรับโครงสร้าง tab ใหม่ทันที (ตรงในเซสชันนั้น ไม่ผ่าน formal Product/Design spec): ยกเลิกแนวคิดฟีดแยกตามห้องของ WYN-127 เดิม — "โพสต์" กลายเป็น club-wide feed รวมทุกห้อง ส่วนการสร้าง/สลับห้อง (channel) ย้ายไปอยู่ในแท็บ "แชท" (WYN-128) แทน — implement+deploy รอบใหม่นี้เสร็จแล้วเช่นกัน (PR #298 → `deploy-web.yml` run #97, ยืนยัน production จริงแล้ว) ดูรายละเอียดเต็มที่ `.wyn/company/CONTEXT.md` entry "[2026-09-07] WYN-127/128/129 migration + deploy เสร็จ (run #96) แต่ Founder ทดลองใช้จริงแล้วขอปรับโครงสร้าง tab ใหม่ทันที" — schema (`club_channels`/`club_posts.channel_id`) ไม่ต้อง migrate ซ้ำ ยังใช้ตัวเดิม
...
Handoff: ดูขั้นตอน deploy แบบละเอียด (ลำดับ migration, การตรวจสอบหลัง deploy, rollback plan) ที่ `.wyn/logs/deployments/2026-09-07-wyn-127-128-129-club-discord-identity-go-live-package.md` — Task นี้ย้ายไป `.wyn/tasks/completed/` ได้ก็ต่อเมื่อ Founder ยืนยันใช้งานจริงบน production แล้วเท่านั้น (ตาม `.wyn/company/WORKFLOW.md`)
