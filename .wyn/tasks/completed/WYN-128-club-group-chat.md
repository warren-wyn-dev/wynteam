...
Status: **LIVE ใน production (2026-09-07)** — migration + deploy run #96 เสร็จโดย session ที่รับช่วงต่อจาก go-live package (report-a-message + staged-rollout gate ยืนยันจริง) จากนั้น Founder ทดลองใช้จริงด้วย developer account แล้วสั่งปรับโครงสร้างทันที: **"แชท" กลายเป็นแท็บหลักแยกต่างหากของ Club** (ไม่ใช่ toggle ซ้อนอยู่ใน Posts tab อีกต่อไป) — เป็นที่สร้าง/สลับห้อง (channel) ทั้งหมด, unread badge ย้ายมาไว้ที่นี่, badge สมาชิกกลายเป็นจุดสีบน chip แบบ Discord จริง — implement+deploy รอบใหม่เสร็จแล้ว (PR #298 → `deploy-web.yml` run #97, ยืนยัน production จริงแล้ว) ยังอยู่ใน staged-rollout เดิม (เห็นเฉพาะ developer account) — ดูรายละเอียดเต็มที่ `.wyn/company/CONTEXT.md` entry เดียวกับ WYN-127 — schema (`club_channel_messages`/`club_channel_message_reads`) ไม่ต้อง migrate ซ้ำ
Owner: AI Product Manager → AI Design (เสร็จ) → Founder อนุมัติสถาปัตยกรรม (เสร็จ) → AI Coding (เสร็จ) → AI QA & Security (เสร็จ, FAIL) → AI Debug Engineer (เสร็จ) → AI QA & Security (เสร็จ, PASS) → AI Deploy & DevOps (เสร็จ, deploy จริงแล้ว run #96) → Founder ทดลองใช้จริง+สั่งปรับ tab structure (เสร็จ, deploy run #97) → **LIVE**
