...
## Phase 2 — Home interactions (active)
...
Implemented on `feat/wyn-158-phase2-home-interactions` / PR #412:
...
- Publication operation IDs and deterministic object paths are retained for ambiguous network outcomes; `drop_id_for_publication()` reconciles a lost response before retry so the browser does not knowingly create a duplicate Drop or delete committed publication assets.
- Browser-native text and image rendering stays in place; no Apple/SF Pro font files are bundled.

### Phase 2 acceptance criteria

- Consumer Web lint, TypeScript, production build and font-license checks pass.
