...
**อนุมัติแล้ว (2026-09-19 รอบ 5-6) — พร้อมส่ง AI Coding, 3 จุด**:
...
3. **[อนุมัติรอบ 6 — เพิ่มสโคป]** สลับลำดับปุ่มในหน้า `WelcomeScreen`: ย้ายปุ่ม "เข้าสู่ระบบ" (`onClick={()
   => router.push("/login")}`) มาไว้ **ก่อน** ปุ่ม "เข้าสู่ระบบด้วย Google" (เดิม Google อยู่ก่อน) — ทั้งสอง
   ปุ่มยังเป็น `.btn-outline` เหมือนเดิม แค่สลับตำแหน่งใน JSX ทั้งสองจุดใน `WelcomeScreen` (ทั้ง state ปกติ
   และ state `gate === "blocked"` ถ้ามีปุ่มลักษณะเดียวกันอยู่ — ดูโค้ดจริงว่ามีกี่จุดต้องสลับ)

**หมายเหตุ**: โลโก้ WYNOS ที่ใช้ในมอคอัพอาร์ตบอร์ด 4 (จากไฟล์ที่ Founder ส่งมา) เป็นของจริงที่โค้ดใช้อยู่แล้ว
(`/wynos_logo_mark.png`) — ไม่ใช่งานใหม่ที่ต้องแก้

ส่งต่อ AI Coding: แก้ 3 จุดข้างต้น, รัน visual/parity regression suite ที่มีอยู่แล้ว (ดู
