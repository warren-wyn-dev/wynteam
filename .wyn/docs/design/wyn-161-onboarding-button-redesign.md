...
ทุกอย่างใหม่หมด" → เริ่มจากหน้าจอ Onboarding/Login ตามที่ Founder ระบุ
Artifact: https://claude.ai/artifact/Gq2encfg9hTqbrAHJ45o7x (อาร์ตบอร์ด 1 = สี, อาร์ตบอร์ด 2 = ทรง)
Decision log: `.wyn/company/DECISIONS.md` (2026-09-19, WYN-161)
...
ด้วย ink + red signal และเปลี่ยนทรงปุ่มจาก pill เต็ม (M3 default) เป็น rounded-rect 16px — เป็นจุดเริ่มของ
button system ใหม่ทั้ง WYNOS (หน้าจอถัดไปจะตามมาทีหลัง คนละรอบ)
...
สำหรับ action รองแบบ "เข้าชมโดยไม่ login" ที่มีอยู่แล้ว):
...
4. **Text (quiet)** — "เข้าชม WYNOS ได้เลย" (guest browsing, ปิดใช้งานชั่วคราวตาม DECISIONS.md 2026-09-08),
   "ส่งรหัสอีกครั้ง"
   - ไม่มีพื้น/ขอบ, ตัวหนังสือ `#E11D48` (red signal — เดิมเป็น `graphite`/`onSurfaceVariant`) น้ำหนัก 600,
     สูงที่เห็น 36-40px แต่ขยาย hit-area โปร่งใสให้ครบ ≥44px เสมอ (pattern เดียวกับ `action_metric.dart`)
...
|---|---|---|---|
| Default | พื้น ink, ตัวหนังสือ paper | ขอบ+ตัวหนังสือตามระดับ | ตัวหนังสือ red |
...
- Touch target ทุกปุ่ม ≥44×44px จริง (ปุ่ม text ขยาย hit-area โปร่งใส) — ตรวจซ้ำด้วย widget test แบบ
  WYN-106
- Contrast: paper บน ink = **19.7:1 ✅**, ink บน paper (secondary/outline) = **19.7:1 ✅**, red `#E11D48`
  บน paper (ปุ่ม text) = **4.7:1 ✅** (พอดีเกณฑ์ AA ตัวหนังสือปกติ — ถ้าลดขนาด/น้ำหนักตัวหนังสือในอนาคตต้อง
  เช็คซ้ำ)
...
2. สีแดง `#E11D48` (ค่าเดียวกับ `WynColors.likeLight`) ใช้เป็น signal accent ของปุ่ม text/tertiary เท่านั้น
   ห้ามใช้เป็นพื้นปุ่มขนาดใหญ่ (คงกติกาเดิมที่ยังใช้สีแดงนี้เป็นความหมาย "ผิดพลาด/destructive" ในที่อื่นของ
   แอปอยู่ — จุดนี้เป็นการยืมค่าสีเดียวกันมาใช้ในความหมาย "ทางเลือกรอง" ของ Onboarding เท่านั้น ไม่ใช่การ
   เปลี่ยนความหมายสีแดงเดิม)
...
1. อนุมัติ spec นี้ทั้งหมด (สี ink+red, ทรง 16px, ขนาด, states) ให้ส่ง AI Coding แก้ 5 หน้าจอ Onboarding ได้
   เลยหรือไม่
