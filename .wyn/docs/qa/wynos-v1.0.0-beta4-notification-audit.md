...
| K-2 | **ผู้ใช้ Android ได้รับ push ตามปกติแล้ว** — WYNOS ปล่อยเป็นเว็บอย่างเดียว (ไม่มี workflow ไหน build APK เลย) Android จึงเข้าผ่าน Chrome ซึ่งรองรับ Web Push อยู่แล้ว ใช้ service worker · FCM · Edge Function ตัวเดียวกับ iOS และบันทึก `platform = 'web'` เหมือนกัน · ง่ายกว่า iOS ด้วยซ้ำ เพราะ Chrome ไม่บังคับติดตั้งลงหน้าจอโฮมก่อน · **ข้อที่ยังค้างคือ APK เนทีฟเท่านั้น** (`google-services.json` + Gradle plugin) ซึ่งยังไม่มีใครสร้างและไม่มีท่อ deploy รองรับ — ถ้อยคำเดิมของข้อนี้อ่านแล้วเข้าใจผิดว่าผู้ใช้ Android ไม่ได้รับ push | ต่ำ (เฉพาะ APK เนทีฟ) | Push บน Android จะยังไม่ทำงานจนกว่าจะทำข้อ 2 ข้างบน — เจตนาเดิมตั้งแต่ WYN-016 |
