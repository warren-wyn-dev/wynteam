...
| # | เรื่อง | ความรุนแรง |
|---|---|---|
| K-1c | **Edge Function ที่ Beta4 แก้ ยังไม่ได้ deploy** — production ยังรัน function ตัวเก่า (ไม่มี collapse key กัน notification ซ้ำ) · ของเดิมยังทำงานปกติทุกอย่าง แค่ยังไม่มี dedupe · เพิ่ม `.github/workflows/deploy-edge-functions.yml` ให้กดปุ่มเดียวได้แล้ว · รันจริงแล้ว 1 ครั้ง (run `33776152809`) ผ่านทุกขั้นรวม `deno check`/`deno test` และดึง project ref ถูกต้อง ล้มที่ขั้นเดียวคือ `Access token not provided` · **เหลือแค่ Founder ใส่ secret `SUPABASE_ACCESS_TOKEN` แล้วกดรันซ้ำ** | กลาง |
