...
- **Current Features**: WYN-002 (Authentication & Onboarding), WYN-003 (User Profile — view/edit display name, bio, avatar), และ WYN-004 (Feed & Post — global feed, ข้อความ+รูป, Like, Comment, ลบโพสต์ตัวเอง) **ทั้งสามผ่าน QA (PASS ระดับโค้ด/static)** แล้ว (ดู `.wyn/tasks/approved/`) ยังห้าม deploy จริงจนกว่าจะมี Supabase project จริง, native platform config, และ `flutter build`/dynamic test ผ่านจริง — งานถัดไปคือ AI Deploy & DevOps เมื่อ infra พร้อม
- **Product Direction — เปลี่ยนแล้ว (2026-08-14)**: Founder แทนที่ทิศทางเดิมทั้งหมดด้วย spec ใหม่ "WYN V0.1 — CORE APP FEATURE PROMPT" — โครงสร้างใหม่คือ Bottom Nav 4 เมนู Home (Search+Feed รวม)/Drop (โพสต์รูปภาพ)/Pop (โพสต์คลิปสั้นแนวตั้ง)/Profile พร้อม Like/Comment/Share/Save/Follow/Notification เต็มรูปแบบ — ดูรายละเอียดเต็มที่ `.wyn/company/DECISIONS.md` (2026-08-14) WYN-002/WYN-003 ยังใช้เป็นฐานได้ (Auth/Profile ตรงกับ spec ใหม่) แต่ WYN-004 (Feed & Post แบบรวม) ถูกแทนที่ด้วย Drop+Pop แยกกัน (โค้ดเดิมของ WYN-004 ยังไม่ถูกลบ แต่ไม่มี route ชี้ไปแล้ว — รอ Founder/Product ตัดสินใจว่าจะลบหรือ repurpose ตอนทำ WYN-007) — AI Product Manager วาง roadmap เต็มไว้ที่ `.wyn/docs/product/wyn-v0.1-roadmap.md` (9 task ใหม่: WYN-005 ถึง WYN-013) **WYN-005 (Drop) QA รอบ 1 — FAIL** (Major: ไม่มีปุ่ม "Like Comment" เลยทั้งที่ Product+Design spec ระบุไว้ตรงกันทั้งคู่) **AI Debug Engineer แก้แล้ว** (เพิ่มตาราง `drop_comment_likes`, `DropComment.likeCount`/`likedByMe`/`toggledLike()`, `DropRepository.toggleCommentLike`, ปุ่ม Like เล็กข้างคอมเมนต์ พร้อม regression test ที่พิสูจน์ fail จริงก่อนแก้/pass หลังแก้) **QA รอบ 2 — FAIL อีกครั้ง** (Major ใหม่: ไม่มีทางลบ Comment ของตัวเองได้เลย ทั้งที่ Product spec ระบุไว้ตรงทั้ง Requirements และ Acceptance Criteria — DB มี RLS delete policy รองรับไว้แล้วแต่ไม่มี repository method/UI เลย root cause เดียวกับบั๊กรอบ 1) รอ AI Debug Engineer แก้รอบ 2 — ดู `.wyn/tasks/bugs/WYN-005-drop-post-image.md`
