...
- **Current Features**: WYN-002 (Authentication & Onboarding) — โค้ดที่ `app/` ผ่าน `flutter analyze` (no issues) และ `flutter test` (2/2 passed) แล้ว แต่ยังไม่เคย `flutter build` (ไม่มี Android SDK/Xcode ในเครื่องที่ตรวจสอบ) และยังไม่ผ่าน QA end-to-end กับ Supabase project จริง ห้ามถือว่าใช้งานได้จนกว่า QA จะ PASS
