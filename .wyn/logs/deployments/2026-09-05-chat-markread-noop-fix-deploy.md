...
### Production verification — **ยังไม่ยืนยัน (ต้องรอ Founder เช็คด้วยตาอีกครั้ง — สำคัญ: ต้อง reload หน้าใหม่จริงๆ)**

**สภาพแวดล้อมของ session นี้ถูก block ไม่ให้ออกเน็ตไปที่ `wynos.online`** เหมือนทุกรอบ —
ยืนยันได้แค่ว่า workflow รายงาน success ทุก step และข้อมูลจริงในฐานข้อมูลตรงกับที่คาดไว้

ขอให้ Founder ทดสอบใหม่โดย **ปิด-เปิดแท็บ/แอปใหม่จริงๆ ก่อน** (ไม่ใช่แท็บที่ค้างจากรอบก่อน
เพราะ Flutter Web โหลดโค้ดครั้งเดียวตอนเปิดหน้า) แล้วทำตามนี้:
1. กดไอคอนข้อความมุมขวาบน Home
2. เข้า inbox → กดเข้าไปอ่านบทสนทนาที่มีข้อความยังไม่อ่าน
3. กดย้อนกลับ 2 ครั้งจนถึง Home
4. เลข badge ต้องลดลง/หายไปจริง
