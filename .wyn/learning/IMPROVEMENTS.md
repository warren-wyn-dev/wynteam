...
- **อัปเดต [2026-08-16] — พบว่า sandbox มี local Postgres ใช้ได้จริง**: ระหว่าง QA verification ของ SCHEMA-001's fix พบว่า environment นี้มี `postgresql-16` ติดตั้งไว้อยู่แล้ว (ไม่เคยมีใครลองมาก่อน) — QA จึง stub ส่วนของ Supabase platform ที่ `schema.sql` สมมติว่ามีอยู่แล้ว (`auth.users`/`auth.uid()`/`auth.role()`, `storage.buckets`/`storage.objects`/`storage.foldername()`, role `authenticated`) แล้วรัน `psql -f supabase/schema.sql` จริงกับ DB ว่างเปล่าสด ๆ ได้สำเร็จ (0 error) พร้อม negative control ยืนยันว่า pre-fix version ยัง reproduce error เดิมของ Founder ได้แม่นยำ (`relation "public.clubs" does not exist`) — เป็นครั้งแรกที่ `schema.sql` review ในโปรเจกต์นี้ backed ด้วยการรันจริงแทนการอ่าน SQL อย่างเดียว ควรเช็คว่ามี local Postgres ใช้ได้ก่อนเสมอเวลาต้องรีวิว `schema.sql` รอบถัดไป แทนที่จะสมมติว่าไม่มีเหมือนที่ผ่านมาทุกครั้ง
- สถานะ: นำไปใช้แล้ว (บางส่วน — ใช้ static script `check_schema_ordering.py` เป็นประจำแล้ว, การรันจริงผ่าน local Postgres ยังเป็น manual step ที่ QA ทำเองรอบนี้ ยังไม่ automate)
