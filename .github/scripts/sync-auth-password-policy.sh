...
if printf '%s' "$existing" | tr ',' '\n' | grep -Fxq "$CALLBACK"; then
  updated="$existing"
elif [ -n "$existing" ]; then
  updated="$existing,$CALLBACK"
else
  updated="$CALLBACK"
fi
...
jq -e --arg cb "$CALLBACK" \
  '(.password_min_length >= 12) and
   ((.uri_allow_list // "" | split(",") | map(gsub("^\\s+|\\s+$"; "")) | index($cb)) != null)' \
  "$after" >/dev/null || {
    echo "::error::Supabase Auth config failed post-update verification"
    exit 1
  }

echo "Supabase Auth password minimum and confirmation redirect verified"
