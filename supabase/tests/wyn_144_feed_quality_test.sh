#!/usr/bin/env bash
# Regression test for WYN-144 Feed Quality, Anti-Spam & Content Fatigue.
...
DB_NAME="wyn144_feed_quality_regression_test"
WORK_DIR="$(mktemp -d)"
...
-- Central quality is bounded, smoothed, and not a raw-like formula.
insert into results select 'CHECK22_quality_is_bounded',
 (public.calculate_feed_quality_score(50,80,10,10,0,0,0,0) between 0 and 1)::int,1;
insert into results select 'CHECK23_unique_quality_beats_noisy_negative',
 ((public.calculate_feed_quality_score(50,80,10,10,0,0,0,0)) >
  (public.calculate_feed_quality_score(2,80,0,0,20,30,0,0.5)))::int,1;
insert into results select 'CHECK24_tiny_sample_is_smoothed',
 (public.calculate_feed_quality_score(0,0,0,0,1,0,0,0)>0.5)::int,1;
insert into results select 'CHECK25_reports_are_strong',
 (public.calculate_feed_quality_score(50,80,10,10,0,0,3,0)<0.35)::int,1;
insert into results select 'CHECK26_spam_risk_is_bounded_penalty',
 ((public.calculate_feed_quality_score(10,20,2,2,0,0,0,1)) <
  (public.calculate_feed_quality_score(10,20,2,2,0,0,0,0)))::int,1;
select public.refresh_trending_scores(now());
select public.refresh_feed_content_quality(now());
select public.refresh_feed_content_quality(now());
...
insert into results select 'CHECK29_global_rankers_unchanged',
 ((position('feed_content_quality' in pg_get_functiondef(
   ...
   and position('feed_content_quality' in pg_get_functiondef(
   'public.get_top100_candidates(integer)'::regprocedure))=0)::int),1;
insert into results select 'CHECK30_sensitive_quality_fields_not_granted',
 ((has_column_privilege('authenticated','public.feed_content_quality','spam_risk','SELECT')=false
   and has_column_privilege('authenticated','public.feed_content_quality','quality_score','SELECT')=false)::int),1;
