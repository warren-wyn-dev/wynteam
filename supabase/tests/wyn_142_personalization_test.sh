#!/usr/bin/env bash
# Regression test for WYN-142 Personalization & Learning Signals.
...
DB_NAME="wyn142_personalization_regression_test"
WORK_DIR="$(mktemp -d)"
...
-- Mirror Supabase's platform grants so request roles can call auth helpers.
-- This keeps the harness focused on the personalization/RLS behavior rather
-- than failing at schema privilege lookup before auth.uid() is evaluated.
