...
-- ============================================================
-- WYNOS Home reference spec, section 4.8 -- Liked-by stacked avatars
-- ============================================================
--
-- Additive: `liked_by` is a jsonb array of the (up to 3) most recent
-- likers' {username, display_name, avatar_url}, the same cheap-count-
-- style correlated subquery image_count already established on this
-- view, just returning a small json array instead of an integer.
-- Capped at 3 (jsonb_agg over a `limit 3` subquery, not the full liker
-- list) since that's all the design ever renders -- "hidden entirely
-- if total like count is 0" is still like_count's own job, unchanged;
-- this column has nothing to do with whether the row itself renders.
create or replace view public.home_feed
  with (security_invoker = true) as
select
  d.id,
...
-- ============================================================
-- WYNOS Home reference spec, section 4.10 -- Top reply preview
-- ============================================================
--
-- Additive: `top_reply` is a jsonb object (or null) with the single
-- top-level comment (parent_comment_id is null -- a reply-to-a-reply
-- is never surfaced here, same "one level of nesting shown" scope the
-- reference's own mock implies) with the most likes on this Drop/Pop,
-- ties broken by most recent. Null when there are no top-level
-- comments at all -- WynosTopReply (Dart side) renders nothing in
-- that case, never an empty prompt.
create or replace view public.home_feed
  with (security_invoker = true) as
select
  d.id,
