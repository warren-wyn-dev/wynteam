...
replace_once(profile_path, "    this.coverUrl,\n    this.platformRole = PlatformRole.user,\n", "    this.coverUrl,\n    this.socialLinks = const {},\n    this.platformRole = PlatformRole.user,\n")
replace_once(profile_path, "        coverUrl: map['cover_url'] as String?,\n        platformRole: platformRoleFromString(map['platform_role'] as String?),\n", "        coverUrl: map['cover_url'] as String?,\n        socialLinks: _parseSocialLinks(map['social_links']),\n        platformRole: platformRoleFromString(map['platform_role'] as String?),\n")
replace_once(profile_path, "  final String? coverUrl;\n  final PlatformRole platformRole;\n", "  final String? coverUrl;\n  final Map<String, String> socialLinks;\n  final PlatformRole platformRole;\n")
replace_once(profile_path, "/// A WYN user profile row. See supabase/schema.sql (WYN-003 section,\n", "Map<String, String> _parseSocialLinks(Object? value) {\n  if (value is! Map) return const {};\n  final result = <String, String>{};\n  for (final entry in value.entries) {\n    final key = entry.key.toString();\n    final link = entry.value?.toString().trim() ?? '';\n    if (key.isNotEmpty && link.isNotEmpty) result[key] = link;\n  }\n  return result;\n}\n\n/// A WYN user profile row. See supabase/schema.sql (WYN-003 section,\n")

repo_path = "app/lib/features/profile/data/profile_repository.dart"
replace_once(repo_path, "id, username, display_name, bio, avatar_url, cover_url, platform_role, is_private, is_verified, dm_permission, mention_permission, comment_permission, likes_visibility", "id, username, display_name, bio, avatar_url, cover_url, social_links, platform_role, is_private, is_verified, dm_permission, mention_permission, comment_permission, likes_visibility")
replace_once(repo_path, "  Future<void> updateProfile({\n    required String userId,\n    required String displayName,\n    required String bio,\n  }) {\n", "  Future<void> updateProfile({\n    required String userId,\n    required String displayName,\n    required String bio,\n    Map<String, String>? socialLinks,\n  }) {\n")
replace_once(repo_path, "      'display_name': normalizeOptionalText(displayName),\n      'bio': bio,\n    }).eq('id', userId);\n", "      'display_name': normalizeOptionalText(displayName),\n      'bio': bio,\n      if (socialLinks != null) 'social_links': socialLinks,\n    }).eq('id', userId);\n")
...
replace_once(recording_path, "  Future<void> updateProfile({\n    required String userId,\n    required String displayName,\n    required String bio,\n  }) async {\n", "  Future<void> updateProfile({\n    required String userId,\n    required String displayName,\n    required String bio,\n    Map<String, String>? socialLinks,\n  }) async {\n")

test_path = "app/test/edit_profile_screen_test.dart"
test = read(test_path).replace("/160", "/150")
write(test_path, test)

schema_path = "supabase/schema.sql"
replace_once(schema_path, "  add column if not exists avatar_url text,\n  add column if not exists cover_url text;\n", "  add column if not exists avatar_url text,\n  add column if not exists cover_url text,\n  add column if not exists social_links jsonb not null default '{}'::jsonb;\n")
