...
/// Poll display + voting widget (WYN-115, Design Screen 2) -- shared
/// verbatim between ClubPostCard and ClubPostDetailScreen, replacing
/// the image/link area for a Poll Club Post. Deliberately dumb/
/// stateless: [myVoteIndex]/[totalVotes]/[optionCounts] all come from
/// the parent (which owns the optimistic-update-then-revert-on-error
/// dance, same as Like/Save), this widget just renders whatever it's
/// given and reports taps via [onVote]. Mirrors Drop's [PollCard]
/// (WYN-035) exactly -- both read `Theme.of(context).colorScheme`,
/// which resolves to the same Sapphire `primary` app-wide, so no
/// Club-specific recoloring is needed.
class ClubPollCard extends StatelessWidget {
  const ClubPollCard({
    super.key,
  ...
  /// `get_club_poll_results()` in supabase/schema.sql.
...
class _ClubPollOption extends StatelessWidget {
  const _ClubPollOption({
    required this.text,
