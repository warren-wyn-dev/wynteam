...
class _OverlappingPageHomeRepository extends RecordingHomeRepository {
  _OverlappingPageHomeRepository({required this.page0, required this.page1})
...
void main() {
  ...
  setUpAll(() async {
    ...
    emptyFromClubsPostRepository = RecordingClubPostRepository(
      fromJoinedClubs: [],
    );
    fromClubsPostRepository = RecordingClubPostRepository(
      fromJoinedClubs: [
        ClubPost(
          id: 'cp1',
          clubId: 'club-1',
    ...
    pollVoteTestHomeRepository = RecordingHomeRepository(
      feedItems: [_pollItem()],
    );
    pollResultsVisibleTestHomeRepository = RecordingHomeRepository(
      feedItems: [
    ...
    pollVoteFailTestHomeRepository = RecordingHomeRepository(
      feedItems: [_pollItem()],
    );
    ...
    verifiedAuthorTestHomeRepository = RecordingHomeRepository(
      feedItems: [_dropItem(id: 'v1', hasImage: false, authorIsVerified: true)],
    );
  ...
  Widget buildHome(
    ...
    home: HomeFeedScreen(
      ...
      homeTabActivatedSignal: homeTabActivatedSignal ?? ValueNotifier<int>(0),
    ),
  );
  ...
  testWidgets(
    'HomeDropCard shows a relative post timestamp beside the author name '
    '(WYN-023 R1)',
    (tester) async {
      await tester.pumpWidget(
        buildHome(
  ...
  testWidgets('rapid double-tap on a Drop card\'s Like routes through '
    ...
    expect(dropLikeTestDropRepository.toggleLikeCurrentlyLikedArgs, [
      false,
      true,
    ]);
  ...
  testWidgets('rapid double-tap on a Pop card\'s Like routes through '
    ...
    expect(popLikeTestPopRepository.toggleLikeCurrentlyLikedArgs, [
      false,
      true,
    ]);
  ...
  group('ReDrop action sheet (WYN-034)', () {
    testWidgets('Beta4 §6: tapping the ReDrop icon opens an ActionSheetBody '
    ...
    testWidgets('the ReDrop button is hidden entirely (not disabled) on a card '
    ...
    testWidgets('tapping "รีโพสต์" in the sheet calls toggleRedrop with '
  ...
  group('"Hide" User Signal (WYNOS Unified Home Feed Algorithm V1.0)', () {
    testWidgets(
      ...
      (tester) async {
        ...
        expect(hideDropTestHomeRepository.hideContentArgs, [
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        expect(hidePopTestHomeRepository.hideContentArgs, [
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        expect(find.text('แคปชัน Drop'), findsOneWidget);
        expect(hideDropTestHomeRepository.unhideContentArgs, [
          (HomeContentType.drop, 'hide-d1'),
        ]);
      },
    );
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        expect(find.text('แคปชัน Drop'), findsNothing);
        expect(
          hideDropUndoTimeoutTestHomeRepository.unhideContentArgs,
          isEmpty,
        );
      },
    );
  ...
  testWidgets('tapping the Comment icon on a Drop card opens DropDetailScreen, '
    ...
    final commentButton = find.widgetWithIcon(
      ActionMetric,
  ...
  testWidgets(
    ...
    'with the comment sheet already showing (WYN-023 R2)',
    (tester) async {
      await tester.pumpWidget(
        buildHome(
  ...
  testWidgets('tapping a Pop card itself (not its Comment icon) opens '
  ...
  group('"สำหรับคุณ"/"Club" feed toggle (WYN-015)', () {
    ...
    testWidgets('shows a join-prompt message on "Club" when the user has no '
    ...
    testWidgets(
      ...
      'and reloads on return (WYN-023 R3)',
      (tester) async {
        await tester.pumpWidget(
          buildHome(
            mixedFeedHomeRepository,
  ...
  group('Feed mode swipe gesture (WYN-140)', () {
    ...
    testWidgets('a decisive leftward swipe on the feed switches "สำหรับคุณ" -> '
      ...
      expect(
        swipeTestHomeRepository.fetchFollowingFeedCalls,
        callsBefore + 1,
        reason:
            'swiping left should have loaded "ติดตาม" the same way '
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        expect(
          ...
          closeTo(1, 0.01),
          reason:
              'swiping right from Club should land on ติดตาม, one step '
    ...
    testWidgets(
      ...
      'clean no-op -- no crash, no page change',
      (tester) async {
        await tester.pumpWidget(
          buildHome(
            swipeTestHomeRepository,
        ...
        expect(
          ...
          closeTo(0, 0.01),
          reason:
              'already at the first mode -- a swipe past the start '
              'should be a no-op',
        );
        expect(find.text('แคปชัน Drop'), findsOneWidget);
      },
    ...
    testWidgets('a short, slow drag well under the velocity threshold does not '
      ...
      expect(
        ...
        closeTo(0, 0.01),
        reason:
            'a slow, short drag should not read as a deliberate '
            'swipe',
      );
      expect(find.text('แคปชัน Drop'), findsOneWidget);
    });
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        expect(
          ...
          findsOneWidget,
          reason:
              'imageCount: 3 should have built the real carousel, not '
        ...
        final carouselController = tester
            .widget<ListView>(carouselListView)
            .controller!;
        ...
        expect(
          carouselController.position.pixels,
          greaterThan(0),
        ...
        expect(
          ...
          closeTo(0, 0.01),
          reason:
              'a drag that started on the carousel must not also page '
  ...
  group('Home feed ranking (WYN-018)', () {
    ...
    testWidgets('only 3 segments ("สำหรับคุณ"/"ติดตาม"/"Club") are present '
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        expect(
          ...
          lessThan(25),
          reason:
              'label must stay a single line (ellipsis-truncated if it '
    ...
    testWidgets('the short segment label ("ติดตาม") is fully legible, not '
      ...
      for (final label in ['ติดตาม']) {
        ...
        expect(
          ...
          isFalse,
          reason:
              '"$label" is short enough that it should render fully, '
        ...
        (tester) async {
          ...
          for (final label in ['สำหรับคุณ', 'ติดตาม', 'Club']) {
            ...
            expect(
              ...
              isFalse,
              reason:
                  '"$label" must never be truncated at ${width}px -- that '
          ...
          expect(
            find.byKey(const Key('active_segment_accent')),
            findsOneWidget,
          );
        },
      );
  ...
  group('Tap Home Tab to Scroll to Top & Refresh (WYN-064)', () {
    ...
    testWidgets(
      'reselecting Home while already at the top triggers a refresh',
      (tester) async {
        final reselectSignal = ValueNotifier<int>(0);
  ...
  group('WYN-088: view-count icon (Wynos V1.0.0 Beta2, item 27)', () {
    ...
    testWidgets('HomeDropCard shows the view-count icon by default (i.e. '
  ...
  group('WYN-093: dynamic-height/aspect-fit images (Wynos V1.0.0 Beta2, '
      'item 19)', () {
    ...
    testWidgets('a portrait image within the 4:5 (0.8) .. 1.91:1 clamp range '
    ...
    testWidgets('a landscape image within the clamp range renders at its true '
    ...
    testWidgets(
      ...
      '0.8, not rendered at its uncropped extreme ratio',
      (tester) async {
    ...
    testWidgets('an extremely wide image (e.g. a panorama) is clamped to 1.91, '
    ...
    testWidgets('a Drop with no image_width/image_height metadata (uploaded '
        'before this migration) falls back to the old fixed 1:1 square', (
      tester,
    ) async {
      await pumpCard(tester, _dropItem(imageWidth: null, imageHeight: null));
      await tester.pumpAndSettle();
      tester.takeException();
    ...
    testWidgets(
      ...
      'viewport on a very tall screen',
      (tester) async {
  ...
  group('WYN-092: multi-image peek carousel (Wynos V1.0.0 Beta2 Phase 2, '
      'item 14)', () {
    ...
    testWidgets('a single-image Drop (imageCount 1 or null) never builds the '
    ...
    testWidgets('a multi-image Drop (imageCount > 1) shows the first image '
    ...
    // right one.
    testWidgets('every section of a card starts on the same left edge as the '
    ...
    testWidgets('once fetched, a multi-image Drop shows every image at 82% of '
      ...
      expect(firstImageSize.height, closeTo(firstImageSize.width * 5 / 4, 0.5));
    ...
    testWidgets('a failed fetchDropImages falls back to showing just the first '
    ...
    testWidgets('a single tap (no second tap) on the carousel opens Detail via '
  ...
  group('"..." menu Share/Save (WYNOSHomeSpec.md 4.6)', () {
    testWidgets('the "..." menu is shown even on the viewer\'s own plain Drop, '
    ...
    testWidgets(
      'tapping "บันทึก" in the menu calls DropRepository.toggleSave',
      (tester) async {
        await tester.pumpWidget(
          buildHome(
            moreMenuTestHomeRepository,
        ...
        await tester.tap(find.widgetWithIcon(IconButton, Icons.more_horiz));
        await tester.pumpAndSettle();
  ...
  group('New-posts pill (WYNOSHomeSpec.md 4.4)', () {
    testWidgets('hidden by default, appears once someone else posts, with the '
      ...
      expect(tester.widget<NewPostsPill>(find.byType(NewPostsPill)).count, 1);
    });
    ...
    testWidgets('each new post from someone else increments the count', (
      ...
      expect(tester.widget<NewPostsPill>(find.byType(NewPostsPill)).count, 2);
    });
  ...
  group('feed integrity under real pagination/tap behaviour (Beta2 audit)', () {
    testWidgets(
      ...
      (tester) async {
        ...
        expect(
          overlappingPageHomeRepository.fetchRankedFeedCalls,
          2,
    ...
    testWidgets('a second Like tap while the first write is still in flight is '
      ...
      expect(
        pendingLikeDropRepository.toggleLikeCalls,
        1,
      ...
      expect(
        pendingLikeDropRepository.maxConcurrentWrites,
        1,
        reason: 'the two writes never overlapped',
      );
    });

    testWidgets('coming back from Detail refreshes only that card, keeping the '
      ...
      backFromDetailHomeRepository.itemsById['${card.item.id}:'] = card.item
          .copyWith(likeCount: card.item.likeCount + 1);
      ...
      expect(
        backFromDetailHomeRepository.fetchItemByIdCalls,
        1,
        reason: 'just the one card is re-read',
      );
      expect(
        backFromDetailHomeRepository.fetchRankedFeedCalls,
        fetchesBefore,
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        expect(
          find.byKey(const Key('home_feed_load_more_retry')),
          findsNothing,
        );
      },
    );
