...
void main() {
  ...
  group('Conversation.fromMap', () {
    ...
    test(
      'parses status and requestedBy (WYN-032) for a pending conversation',
      () {
        final conversation = Conversation.fromMap(
          baseMap(status: 'pending', requestedBy: 'me'),
        );
    ...
    test('parses every field', () {
      ...
      expect(
        ...
        DateTime.parse('2026-08-22T10:00:00Z'),
      );
    ...
    test('sortKey uses lastMessageAt once a message exists', () {
      final conversation = Conversation.fromMap(
        baseMap(lastMessageAt: '2026-08-22T10:00:00Z'),
      );
    ...
    group('isUnread', () {
      ...
      test(
        'true when never read at all (myLastReadAt null) but a message exists',
        () {
          final conversation = Conversation.fromMap(
            baseMap(
              lastMessageAt: '2026-08-22T10:00:00Z',
  ...
  group('ChatMessage.fromMap', () {
    ...
    test(
      'a soft-deleted message has null text/image and a non-null deletedAt',
      () {
        final message = ChatMessage.fromMap({
          'id': 'm1',
          'conversation_id': 'c1',
    ...
    test(
      'WYN-138: parses edited_at, and isEdited is false when it is null',
      () {
