...
void main() {
  ...
  setUpAll(() async {
    ...
    errorRepo = RecordingDropRepository()
      ..fetchLikedByAuthorError = Exception('boom');
  ...
  testWidgets('shows every Drop returned by fetchLikedByAuthor as a full-width '
  ...
  group('WYN-099: distinguishing "no Likes yet" from "not allowed to see '
      'this"', () {
    testWidgets('an empty list + canViewLikes true shows the ordinary empty '
    ...
    testWidgets('an empty list + canViewLikes false shows the privacy-blocked '
    ...
    testWidgets('a non-empty list never calls canViewLikes at all -- the RPC '
  ...
  group('Beta3 -- coming back from Detail', () {
    testWidgets(
      ...
      (tester) async {
        ...
        repo.fetchByIdResults['d1'] = _drop(id: 'd1').copyWith(likeCount: 9);
    ...
    testWidgets('drops a row that was unliked while Detail was open', (
      ...
      repo.fetchByIdResults['d1'] = _drop(id: 'd1')
          .copyWith(likedByMe: false, likeCount: 0);
  ...
  testWidgets(
    ...
    'next page exactly once, not several times',
    (tester) async {
      ...
      expect(
        repo.fetchLikedByAuthorCalls,
        2,
        reason:
            'page 0 (initial) + page 1 (exactly one pagination fetch) '
...
class _CountingCanViewLikesProfileRepository
    extends RecordingProfileRepository {
